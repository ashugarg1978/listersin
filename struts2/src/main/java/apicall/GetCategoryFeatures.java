package ebaytool.apicall;

import com.mongodb.Mongo;
import com.mongodb.DB;
import com.mongodb.DBCollection;
import com.mongodb.BasicDBObject;
import com.mongodb.DBObject;
import com.mongodb.DBCursor;
import com.mongodb.util.*;

import ebaytool.apicall.ApiCall;

import java.io.*;
import java.net.URL;
import java.util.concurrent.*;
import javax.net.ssl.HttpsURLConnection;

import net.sf.json.JSONObject;
import net.sf.json.JSONArray;
import net.sf.json.xml.XMLSerializer;

import java.util.HashMap;
import java.util.*;

public class GetCategoryFeatures extends ApiCall implements Callable {
	
	private Integer siteid;
	private String  site;
	
	public GetCategoryFeatures (Integer siteid, String site) {
		this.siteid = siteid;
		this.site   = site;
	}
	
	public String call() throws Exception {
		
		String token = "AgAAAA**AQAAAA**aAAAAA**KHmBTA**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6wFk4CoD5mKpw2dj6x9nY+seQ**Q0UBAA**AAMAAA**";
		
		BasicDBObject requestdbobject = new BasicDBObject();
		requestdbobject.append("RequesterCredentials", new BasicDBObject("eBayAuthToken", token));
		requestdbobject.append("WarningLevel", "High");
		requestdbobject.append("DetailLevel",  "ReturnAll");
		requestdbobject.append("ViewAllNodes", "true");
		
		JSONObject requestjsonobject = JSONObject.fromObject(requestdbobject.toString());
		
		XMLSerializer xmls = new XMLSerializer();
		xmls.setObjectName("GetCategoryFeatures");
		xmls.setNamespace(null, "urn:ebay:apis:eBLBaseComponents");
		xmls.setTypeHintsEnabled(false);
		String requestxml = xmls.write(requestjsonobject);
		
		String responsexml = callapi(siteid, requestxml);
		
		writelog("GetCategoryFeatures."+site+".res.xml", responsexml);
		
		JSONObject json = (JSONObject) new XMLSerializer().read(responsexml);
		
		BasicDBObject responsedbo = convertXML2DBObject(responsexml);
		
		Mongo m = new Mongo();
		DB db = m.getDB("ebay");
		
		DBCollection coll = db.getCollection("CategoryFeatures_"+site);
		coll.drop();
		coll.insert((List<DBObject>) responsedbo.get("Category"));
		
		return "";
	}
	
}
